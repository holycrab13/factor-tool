...
function autoComplete(input, list) {
	...
	var query = "http://lookup.dbpedia.org/api/search/PrefixSearch?QueryClass=&MaxHits=10&QueryString=" + encodeURIComponent(input);
...
$("#pin-form").submit(function( event ) {
	...
	if(suggestions.length > 0) {
		...
		$("#pin-title").html(suggestions[0].label);
		var pin = { text: suggestions[0].label, uri: suggestions[0].uri, isClass: true };
...
$("#search-bar").submit(function( event ) {
	...
	if(suggestions.length > 0) {
		...
		var isOntologyClass = suggestions[0].uri.includes("http://dbpedia.org/ontology");

		$("#search-input").val("");



		var pin = { text: suggestions[0].label, uri: suggestions[0].uri, isClass: isOntologyClass };
