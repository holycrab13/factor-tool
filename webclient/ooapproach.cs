...
class QuerySet
{
	...
	string GenerateHeader(QuerySet set)
	{
		// Basic: Thing -> replaced by refinements

		var domain = dbo:Thing;

		foreach(InstanceFilter filter in filters) 
		{
			var filterDomain = filter.

			if(filterDomain is child of domain)
			{
				domain = filterDomain;
			}
			else if(domain !isChildOf filterDomain)
			{
				domain = findCommonAncestors(domain, filterDomain);
			}
		}
	}
...
// Create solr index (name -> uri, type) for properties, classes, resources

// Select with SPARQL-Describe
